...
function renameCategory() {
    ...
    let newCategoryName = $('#rename-category').val().trim();  // Ensure the new category name is trimmed
    let oldCategoryName = selectedRow.find('td').text().trim();  // Ensure the old category name is trimmed

    if (selectedRow.length && newCategoryName) {
        $.ajax({
            url: '/api/updateCategory',
            method: 'PUT',
            data: JSON.stringify({ oldName: oldCategoryName, newName: newCategoryName }),  // Ensure the payload matches the backend expectation
            contentType: 'application/json',
            dataType: 'json',
            success: function (result) {
                if (result.success) {
            ...
            error: function (xhr, status, error) {
                console.error("Error details:", xhr.responseText);
                alert("Error renaming category: " + xhr.responseText);
            }
        });
    } else {
